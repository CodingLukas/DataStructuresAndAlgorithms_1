...
//           kai duotas kampas yra nurodomas laipsniais
    ...
    public void createControls() {
        ...
        addButton("sin",   e -> calculateSin());
        addButton("cos",   e -> calculateCos());
        addButton("tan",   e -> calculateTan());
    }
    ...
    public void calculateCos(){
        int x = readLastInt();
        double rad = Math.toRadians(x);
        printLn(""+Math.cos(rad));
    }
    
    public void calculateTan(){
        int x = readLastInt();
        double rad = Math.toRadians(x);
        printLn(""+Math.tan(rad));
    }
...
//         išbandykite skirtingus ekranų nustatytumus 
        ...
        setTextAreas("green", "red", 300, 400);
