...
public class Demo1_StringBuilder extends BaseConsole {
    ...
    public void createControls() {
        ...
        addButton("pir\u25B2", action -> printPyramid(pyrHeight++, "\u25B2"));
        addButton("pir\u25CF", action -> printPyramid(pyrHeight++, "\u25CF"));
        addButton("pir\u25BC", action -> anotherPyramid(pyrHeight++));
    }
...
// sudarykite pasirinktinai metodus apverstai ar pasuktai piramidei spausdinti
    ...
    private void anotherPyramid(int h) {
        ...
        for (int currHeight = 0; currHeight <= h; currHeight++) {
            StringBuilder pyr1 = new StringBuilder();
            for (int space = 0; space < currHeight; space++) {
                pyr1.append(" ");
            }

            int requiredStars = (h - currHeight) * 2 + 1;
            for (int stars = 0; stars < requiredStars; stars++) {
                pyr1.append(fillSym);
            }
            printLn(pyr1.toString());
        }

    }

    public StringBuilder makeSymbols(StringBuilder builder, String symbol, int times) {
