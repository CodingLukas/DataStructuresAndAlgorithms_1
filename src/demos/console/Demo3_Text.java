...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    ...
    ***************************************************/
    ...
    void lotyniskos(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText("\\p{IsLatin}+<>pirmas=1;antras>-22:trečias:333viskąs-" + nL);
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa)+nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
        Matcher m = pat.matcher(sa[1]);            // kita dalis - apdorojimui
    ...
    void lietuviskai(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText("\\w+[Ą-ž]\\w+<>pirmas=1;antras>-22:trečias:333viskąs-" + nL);
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa)+nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
        Matcher m = pat.matcher(sa[1]);            // kita dalis - apdorojimui
    ...
    void skliaustai(){
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText("\\((.*?)\\)<>pir(mas=1;)antras>-22:(trečias):333viskąs-" + nL);
        String[] sa = readLastLine().split("<>");      // padaliname į dvi dalis
        ta2.appendText(String.join(" : ", sa)+nL); // pakartojame dešinėje
        Pattern pat = Pattern.compile(sa[0]);      // pirma dalis - tai Pattern
        Matcher m = pat.matcher(sa[1]);            // kita dalis - apdorojimui
    ...
    public void createControls() {
        ...
        addButton("minusas",   action -> minusas()); 
        addButton("lotyniskai",   action -> lotyniskos()); 
        addButton("lietuviskai",   action -> lietuviskai()); 
        addButton("Skliaustai", action -> skliaustai());        
    }
    @Override
